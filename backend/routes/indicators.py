...
def api_indicators_intraday():
    try:
        ...
        # psycopg2 returns TIMESTAMPTZ as tz-aware datetime objects (UTC).
        # pd.read_sql wraps them in a DatetimeTZDtype column (datetime64[us, UTC]).
        # Just convert directly to IST — no parsing needed.
        # ── Step 1: FIXED timestamp handling (robust)
        print("Before parse:", len(df))
        # Convert to datetime first (THIS WAS MISSING)
        df["ts"] = pd.to_datetime(df["ts"], format="%d/%m/%Y %H:%M", errors="coerce")
        print("After parse:", df["ts"].notna().sum())
        ...
        # ── Step 2: extract series BEFORE any further df modification
        # This ensures index alignment between series and df is guaranteed.
        close = df["close"].astype(float)
        high = df["high"].astype(float)
        low = df["low"].astype(float)
        volume = df["volume"].astype(float)

        # ── Step 3: minimum candle guard
        ...
        # ── Step 6: ORB per day (09:15–09:20 IST)
        ...
        for current_date in df["ts"].dt.date.unique():
            ...
            win = day[(day["ts"].dt.time >= orb_start) & (day["ts"].dt.time <= orb_end)]
            if win.empty:
                continue
